<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="Default.aspx.cs" Inherits="RIMBA.PI.Default" %>
                            ...
                            <tr><td>
                                <div class="item">
                                    <div class="info oneline">
                                        <div class="col1">
                                            <div class="text-bold"><%# Eval("PurchaseInvoiceID") %></div>
                                        </div>
                                        <div class="col2">
                                    ...
                                    <div>
                                        <asp:Button id="btnCancelApprove" Text="Cancel Approval" CssClass="rounded-button cancel-validation" OnCommand="btnCancelValidation_Command" CommandArgument='<%# Eval("PurchaseInvoiceID") %>' Visible='<%# Convert.ToBoolean(Eval("Approved")) == true %>' runat="server" />
                                        <asp:Button id="btnCancelReject" Text="Cancel Reject" CssClass="rounded-button cancel-validation" OnCommand="btnCancelValidation_Command" CommandArgument='<%# Eval("PurchaseInvoiceID") %>' Visible='<%# Convert.ToBoolean(Eval("Rejected")) == true %>' runat="server" />
                                        <asp:Button id="btnApprove" Text="Approve" CssClass="rounded-button" OnCommand="btnApprove_Command" CommandArgument='<%# Eval("PurchaseInvoiceID") %>' Visible='<%# Convert.ToBoolean(Eval("Approved")) == false && Convert.ToBoolean(Eval("Rejected")) == false %>' runat="server" />
                                        <asp:Button id="btnReject" Text="Reject" CssClass="rounded-button" OnCommand="btnReject_Command" CommandArgument='<%# Eval("PurchaseInvoiceID") %>' Visible='<%# Convert.ToBoolean(Eval("Approved")) == false && Convert.ToBoolean(Eval("Rejected")) == false %>' runat="server" />
                                         <%--<%# Eval("['ValidatedDate']") %>--%>
                                    </div>
                                </div>
                                </td>
