<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="Default.aspx.cs" Inherits="RIMBA.PO.Default" %>
                            ...
                            <tr><td>
                                <div class="item">
                                    <div class="info oneline">
                                        <div class="col1">
                                            <div class="text-bold"><%# Eval("PurchaseOrderID") %></div>
                                        </div>
                                        <div class="col2">
                                    ...
                                    <div class="info oneline">
                                        ...
                                        <div>
                                            <div>Supplier: <%# Eval("SupplierName") %></div>
                                        </div>
                                    </div>
                                        ...
                                        <asp:Repeater ID="rptChild" DataSource='<%# ((System.Data.DataRowView)Container.DataItem).Row.GetChildRows(RELATIONNAME) %>' OnItemCommand="rptChild_ItemCommand" runat="server">
                                            ...
                                            <ItemTemplate>
                                                ...
                                                <div class="child oneline">
                                                    <div class="button">
                                                        <asp:Button ID="btnShowRO" Text="RO" CssClass="rounded-button" CommandArgument='<%# Eval("[No]") %>' runat="server" />
                                                        <asp:Button ID="btnHideRO" Text="Hide" CssClass="rounded-button" Visible="false" runat="server" />
                                                    </div>
                                                    ...
                                                    <div class="harga"><%# string.Format("{0:N2} /{1}", Eval("['Cost']"), Eval("['HighestUnitCode']")) %> </div>   
                                                    <div class="discount"><%# string.Format("{0:N2}%", Eval("['Discount']")) %> </div>
                                                    <div class="total"><%# string.Format("{0:N2}", Eval("['Total']")) %> </div>
                                                </div>

                                                <asp:Panel ID="pnlRO" Visible="false" runat="server">
                                                    <div class="RO">
                                                        <asp:Repeater ID="rptRO"  runat="server">
                                                            <ItemTemplate>
                                                                <div class="oneline">
                                                                    <div class="RO-id"><%# Eval("['RequestOrderID']") %> </div>
                                                                    <div class="RO-amount"><%# string.Format("({0:N0})", Eval("['Quantity']")) %> </div>
                                                                    <div class="RO-notes">Keterangan: <%# Eval("['Detail']") %></div>
                                                                </div>
                                                            </ItemTemplate>
                                                        </asp:Repeater>                     
                                                    </div>
                                    ...
                                    <div>
                                        <asp:Button id="btnCancelApprove" Text="Cancel Approval" CssClass="rounded-button cancel-validation" OnCommand="btnCancelValidation_Command" CommandArgument='<%# Eval("PurchaseOrderID") %>' Visible='<%# Convert.ToBoolean(Eval("Approved")) == true %>' runat="server" />
                                        <asp:Button id="btnCancelReject" Text="Cancel Reject" CssClass="rounded-button cancel-validation" OnCommand="btnCancelValidation_Command" CommandArgument='<%# Eval("PurchaseOrderID") %>' Visible='<%# Convert.ToBoolean(Eval("Rejected")) == true %>' runat="server" />
                                        <asp:Button id="btnApprove" Text="Approve" CssClass="rounded-button" OnCommand="btnApprove_Command" CommandArgument='<%# Eval("PurchaseOrderID") %>' Visible='<%# Convert.ToBoolean(Eval("Approved")) == false && Convert.ToBoolean(Eval("Rejected")) == false %>' runat="server" />
                                        <asp:Button id="btnReject" Text="Reject" CssClass="rounded-button" OnCommand="btnReject_Command" CommandArgument='<%# Eval("PurchaseOrderID") %>' Visible='<%# Convert.ToBoolean(Eval("Approved")) == false && Convert.ToBoolean(Eval("Rejected")) == false %>' runat="server" />
                                         <%--<%# Eval("['ValidatedDate']") %>--%>
                                    </div>
                                </div>
                                </td>
