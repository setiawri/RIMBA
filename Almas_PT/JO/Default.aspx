<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="Default.aspx.cs" Inherits="RIMBA.JO.Default" %>
            ...
            <div class="daftar">
                <table border="0">
                    <asp:Repeater ID="rptParent" runat="server">
                        <ItemTemplate>
                            <tr><td>
                                <div class="item oneline same-height">
                                    <div class="image-container centered">
                                        <img id="img" runat="server" src='<%# getImageSource(Eval("Picture"), Eval("Filename").ToString()) %>' Visible='<%# Eval("Picture") != DBNull.Value %>' alt="" />
                                        <asp:Label ID="lblNoImage" Text="no available image" Visible='<%# Eval("Picture") == DBNull.Value %>' CssClass="center-vertically centered" runat="server" />
                                    </div>
                                    <div class="data">
                                        <div class="info oneline">
                                            <div class="col1">
                                                <div class="text-bold"><%# Eval("ProductionOrderID") %></div>
                                            </div>
                                            <div class="col2">
                                        ...
                                        <div class="grand-total">TOTAL: <%# string.Format("{0:N0}", Eval("QuantityCount")) %></div>
                                        <div>
                                            <asp:Button id="btnApprove" Text="Approve" CssClass="rounded-button" OnCommand="btnApprove_Command" CommandArgument='<%# Eval("ProductionOrderID") %>' Visible='<%# Convert.ToBoolean(Eval("[Approved]")) == false && Convert.ToBoolean(Eval("[Rejected]")) == false %>' runat="server" />
                                            <asp:Button id="btnReject" Text="Reject" CssClass="rounded-button" OnCommand="btnReject_Command" CommandArgument='<%# Eval("ProductionOrderID") %>' Visible='<%# Convert.ToBoolean(Eval("[Approved]")) == false && Convert.ToBoolean(Eval("[Rejected]")) == false %>' runat="server" />
                                            <asp:Button id="btnCancelApprove" Text="Cancel Approval" CssClass="rounded-button cancel-validation" OnCommand="btnCancelValidation_Command" CommandArgument='<%# Eval("[ProductionOrderID]") %>' Visible='<%# Convert.ToBoolean(Eval("[Approved]")) == true %>' runat="server" />
                                            <asp:Button id="btnCancelReject" Text="Cancel Reject" CssClass="rounded-button cancel-validation" OnCommand="btnCancelValidation_Command" CommandArgument='<%# Eval("[ProductionOrderID]") %>' Visible='<%# Convert.ToBoolean(Eval("[Rejected]")) == true %>' runat="server" />
                                        </div>
                                    </div>
                                </div>
                                </td>
